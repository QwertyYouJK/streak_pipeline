...
def flag_edges_at_borders(edges, image_shape, border_margin=10):
    ...
    for edge in edges:
        if (
            edge["x"].min() <= border_margin
            or edge["x"].max() >= image_shape[1] - border_margin
            or edge["y"].min() <= border_margin
            or edge["y"].max() >= image_shape[0] - border_margin
        ):
...
class Streak:
    ...
    def __init__(
        ...
        if remove_bkg not in remove_bkg_options:
            raise RuntimeError(
                '"remove_bkg" must be the one among: %s' % ", ".join(remove_bkg_options)
            )
        ...
        if output_path is None:
            output_path = "%s" % (filename[: filename.rfind(".")])
        if output_path[-1] != "/":
            output_path += "/"
    ...
    def _detect_streaks(self):
        ...
        contours = measure.find_contours(
            self.image,
            self._std * self.contour_threshold,
        ...
        edge = EDGE(
            ...
            shape_cut=self.shape_cut,
            area_cut=self.area_cut,
    ...
    def _detect_sources(self):
        ...
        daofind = DAOStarFinder(
            threshold=(self._med + self._std * detection_threshold), fwhm=fwhm
        )
    ...
    def plot_figures(self, cut_threshold=3.0):
        """
        ...
        if self.streaks is not None:
            ...
            for edge in edges:
                ...
                pl.plot(
                    [xs.min(), xs.max(), xs.max(), xs.min(), xs.min()],
    ...
    def xy2sky(self, filename, x, y, sep=":"):
        ...
        try:
            ...
            coord = "{0} {1}".format(alpha.split(" ")[0], delta.split(" ")[1])
            return coord
    ...
    def xy2sky2(self, filename, x, y):
        ...
        try:
            ...
            astcoords = coordinates.SkyCoord(astcoords_deg * u.deg, frame="icrs")
    ...
    def write_outputs(self):
        ...
        if not self.wcsinfo:
            fp1.writelines(
                "#ID x_center y_center area perimeter shape_factor "
                + "radius_deviation slope_angle intercept "
                + "connectivity\n"
            )
            for n, edge in enumerate(self.streaks):
                line_str = (
                    "%2d %7.2f %7.2f %6.1f %6.1f %6.3f " + "%6.2f %5.2f %7.2f %2d\n"
                )
                line = line_str % (
                    edge["index"],
                    edge["x_center"],
            ...
            fp1.writelines(
                "#ID x_center y_center ra(hms) dec(dms) ra(deg) "
